### Tests: the cosmos3 diffusers live test asserts finite, in-range actions from an embodiment-derived layout
...
finite, the whole raw chunk is asserted finite, within the normalized
`[-1, 1]` range (small eps), and not identically zero, and the MuJoCo tracking
