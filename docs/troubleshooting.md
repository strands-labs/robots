...
| `move_to` reports `EE (body '<robot>/Wrist…')` and low targets fail with a residual of ~0.1 m, or "reach" while the fingers hang above the point | The model ships no tool-point site, so the end-effector frame is a wrist/hand **body** - the SO-100's `Wrist_Pitch_Roll` is ~16 cm behind the jaw tips | Declare the tool point in the registry (`"tool_frame": {"body": …, "pos": […]}` - the shipped `so100` entry has one) or add a `tcp` site to the model; `get_robot_state`'s `end_effector` line then reads `site '<robot>/tcp'` |
