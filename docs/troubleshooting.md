...
| `move_to`/`rotate_wrist`: `... did not reach ... The servo was stopped: the robot is in contact: 'so100/Fixed_Jaw' <-> 'so100/Base' (d=-0.0021 m)` (or `commanded joint(s) at a limit: '...' at its upper limit`) | The set-point was legal, but the servo could not get there: something on the robot is pushing against another geom, or a commanded joint sits on a bound of its range. The refusal names the pair (nearest first, at most three, total in `json.obstruction.contacts_total`) and the joint, read at the final tick | Clear what the robot touches (or command a value that avoids it), or pick one the named joint can reach inside its range. If the text says `no contact involved the robot and no commanded joint was at a limit`, nothing visible blocks it: raise `max_steps` or loosen `tol`. `get_contacts` shows the full list |
