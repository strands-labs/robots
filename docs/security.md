...
- **Payload bounds.** `validate_command` bounds every field of an incoming `execute` / `start` command before it reaches the dispatcher: the action must be in the allowlist, `duration` and `policy_port` are range-checked, `policy_host` / `server_address` / `model_path` are allowlist-gated, and every string field is length-bounded and refused C0/DEL/C1 control characters. That last check is why the audit trail above is worth keeping: `instruction` is free-form text from a remote peer, so admitting a CR or LF in it would let one log call emit two records and let the second impersonate a different level and logger. Natural-language fields bound only the control range - a non-ASCII instruction is admitted - while identifier fields such as `robot_name` stay printable-ASCII-only. Reaching that validator at all means first decoding the caller's `command` string, and `json.loads` refuses one three ways: malformed syntax, a number wider than `sys.get_int_max_str_digits()` (well-formed JSON this build cannot construct an `int` for), and a document nested past the interpreter stack (a `RecursionError`, which is not a `ValueError` at all). All three are decided locally by one decoder and reported through the tool's `{"status": "error"}` envelope with an audit row, so a `broadcast` or `send` refused for its body is still a record - which is what the audit-trail claim above rests on.
...
- **A caller-named REST path segment is one bare name.** The Reachy Mini's recorded-move verbs (`play_move` on the native driver, `playMove` over Device Connect) interpolate `move_name` into `/api/move/play/recorded-move-dataset/{library}/{move}`, so the name is gated to 1-128 chars of `[A-Za-z0-9._-]` *starting with a letter or digit*. The leading-character rule is the part that excludes `.` and `..`: both are spelled entirely from that charset, and `move_name=".."` builds a URL that resolves to the daemon's parent path - an endpoint the caller named nothing about. `list_moves` reports the library's catalogue, which is where a legitimate name comes from. The `{library}` component of that same path is caller-named too, and it is *resolved* rather than interpolated: both drivers look it up in the two-entry set of libraries the daemon serves and refuse a name that is not in it. Resolving through an enumerable set is what keeps the interpolation safe without a second pattern, and it is why an unrecognised library plays nothing instead of falling back to whichever library the expression happened to default to.
...
- **A re-fetch deletes before it fetches, so the flag that asks for one is checked.** `download_robots(force=True)` / `download_assets(force=True)` remove the cached directory of a robot whose assets are already present and fetch it again - the one path in the module that touches files it did not author, since every copy otherwise filters on read so a README or notes kept beside the assets survive. That posture is only as good as how the flag is read, so both surfaces hold `force` to the shared boolean domain rather than reading it by truthiness: a non-boolean is refused before the cache is read, so `force="false"` cannot select the re-fetch it spells the skipping of.
...
- **The `Robot` agent tool asks an operator before it drives real hardware.** `Robot("so100", mode="real")` handed to an `Agent` is the first path to metal in the README, and its `execute` and `start` actions stop for an operator interrupt *before* the rollout is dispatched, through the same gate as `use_ros`; a declined or headless call dispatches nothing, and an agent that has not answered yet is paused rather than moved. Pre-approve by action name with `STRANDS_ROBOT_COMMAND_ALLOW=execute,start` (or `*`) or bypass with `BYPASS_TOOL_CONSENT=true`. `status` and `stop` are never gated, and neither are the observe actions `get_state`/`get_robot_state`, `list_cameras` and `render` - they read the servos and cameras and write nothing, and they exist so an agent asked where the arm is has a read to reach for instead of a rollout (before they existed, the agent's only route to a joint position was `execute` with a mock policy). The simulation tool never is - it touches no hardware. A grant the dashboard's motion hook deposited for the same call is spent instead of asking twice.
