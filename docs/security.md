...
- **Audit trail.** Every `tell` / `send` / `broadcast` / `stop` / `emergency_stop` / `rpc` - and every approval, decline, validation rejection, and rate-limit rejection - is written to the safety audit log. Make sure your deployment actually captures and retains that log; it is your forensic record of what the fleet was told to do.
