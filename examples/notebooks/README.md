...
| 7 | [`07_data_augmentation.ipynb`](07_data_augmentation.ipynb) | Data augmentation at four pipeline stages: seeded domain randomization + sensor noise while recording (`randomize()` / `set_obs_noise()`), backdrop swaps over the same trajectories (`HybridCompositor.set_background`), `TrainSpec.augmentation` mapped to GR00T's image-augmentation flags, and a dataset transform (`create_transform("mock")`) run on CPU against the recorded dataset with the full provenance round trip (`synthetic=true`, `load_provenance` / `synthetic_episode_indices`); the generative `cosmos_transfer` backend needs a GPU, so it stays in prose pointing at `docs/data/transforms.md` |
