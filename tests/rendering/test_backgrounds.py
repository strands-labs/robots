...
# The ``.ply`` reader's decode contract -- the INRIA property layout, the
# log-space/logit activations, the channel-major SH rest block and the two
# refusals -- is pinned in ``tests.rendering.test_ply_gaussian_splat_reader``,
# which reaches it without the GPU-only ``sim-gs`` extra.
