"""``g1_safe_lie_to_stand`` returns exactly what ``G1Driver.safe_lie_to_stand`` gives it.

``g1_safe_lie_to_stand`` is the write-side companion to the
neon bundle's ``g1_safe_lie_to_stand`` verb: where the neon verb
wraps ``LocoClient.Damp`` + ``LocoClient.Lie2StandUp`` under an
...
``G1Driver.safe_lie_to_stand`` method is not yet plumbed today
(refs strands-labs/robots#358 for the SDK-facing gate work the
write belongs on), so the :func:`live_handle_refusal` grader
refuses a handle without a ``safe_lie_to_stand`` accessor with a
...
from strands_robots.tools.g1.g1_safe_lie_to_stand import g1_safe_lie_to_stand


class _StubG1Driver:
    """A driver double whose ``safe_lie_to_stand`` returns a fixed envelope.

    ``g1_safe_lie_to_stand`` calls
    ``driver.safe_lie_to_stand(preamble_s)`` and returns the
    ...
    def safe_lie_to_stand(self, preamble_s: float) -> dict[str, Any]:
...
def _call(
    ...
    *,
    ...
    """
    return g1_safe_lie_to_stand(driver=driver, preamble_s=preamble_s)
...
def test_the_import_pulls_no_sdk_module() -> None:
    ...
    safe-lie-to-stand verb to it too.
    """
    before = set(sys.modules)
    importlib.import_module("strands_robots.tools.g1.g1_safe_lie_to_stand")
    after = set(sys.modules)
    leaked = {name for name in after - before if "unitree" in name.lower()}
    assert leaked == set(), (
        f"strands_robots.tools.g1.g1_safe_lie_to_stand imports pulled SDK "
...
def test_a_driver_side_refusal_surfaces_verbatim() -> None:
    ...
    ``G1Driver.safe_lie_to_stand`` will (once landed) refuse an
    SDK-side raise, an FSM outside ``{1, 702}``  with a named error envelope,
    ...
    """
    refusal_text = (
        "safe_lie_to_stand: FSM=None is not in {1, 702}. Damping outside a controller-managed state risks collapse."
    )
...
def test_a_future_success_envelope_round_trips_verbatim() -> None:
    """Once ``safe_lie_to_stand`` lands, the success envelope surfaces verbatim.

    When the driver's method is plumbed,
    ``G1Driver.safe_lie_to_stand`` will surface the SDK's
    outcome inside a ``{"status": "success", "content": [{"json":
    {"rc": 0, "message": "Damp -> sleep(0.5s) -> Lie2StandUp
        ...
        "content": [
            {
                "json": {
                    "rc": 0,
                    "message": "Damp -> sleep(0.5s) -> Lie2StandUp dispatched",
                }
            }
        ],
    }
...
def test_a_missing_driver_is_refused_with_a_message_naming_the_parameter() -> None:
    ...
    """
    result = g1_safe_lie_to_stand(driver=None, preamble_s=0.5)
    assert result["status"] == "error"
    text = result["content"][0]["text"]
    assert "g1_safe_lie_to_stand" in text
...
def test_a_wrong_shape_driver_is_refused_with_a_message_naming_the_type() -> None:
    ...
    """
    result = g1_safe_lie_to_stand(driver="unitree_g1", preamble_s=0.5)
    assert result["status"] == "error"
    text = result["content"][0]["text"]
    assert "g1_safe_lie_to_stand" in text
    assert "'str'" in text
...
def test_the_verb_writes_the_driver_exactly_once() -> None:
    """One call to the verb makes one call to ``driver.safe_lie_to_stand``.

    A double-call from a wrapper that retried inside the verb
    would issue two Damp+Lie2StandUp writes on the same
...
def test_the_default_preamble_matches_the_neon_bundle_field_notes() -> None:
    ...
    envelope = {"status": "success", "content": [{"json": {}}]}
    driver = _StubG1Driver(envelope=envelope)
    g1_safe_lie_to_stand(driver=driver)
...
def test_the_wrong_shape_driver_is_not_called() -> None:
    ...
    called (``int`` has no ``safe_lie_to_stand`` attribute) and
    observing that the refusal envelope has the four invariants
    without an exception in flight.
    """
    result = g1_safe_lie_to_stand(driver=42, preamble_s=0.5)
    assert result["status"] == "error"
    text = result["content"][0]["text"]
    assert "g1_safe_lie_to_stand" in text
    assert "'int'" in text
...
def test_a_missing_preamble_is_refused_with_a_message_naming_the_parameter() -> None:
    ...
    """
    driver = _StubG1Driver(envelope={"status": "success", "content": [{"json": {}}]})
    result = g1_safe_lie_to_stand(driver=driver, preamble_s=None)  # type: ignore[arg-type]
    assert result["status"] == "error"
    text = result["content"][0]["text"]
    assert "g1_safe_lie_to_stand" in text
...
def test_a_wrong_shape_preamble_is_refused_with_a_message_naming_the_parameter() -> None:
    ...
    """
    driver = _StubG1Driver(envelope={"status": "success", "content": [{"json": {}}]})
    result = g1_safe_lie_to_stand(driver=driver, preamble_s="0.5")  # type: ignore[arg-type]
    assert result["status"] == "error"
    text = result["content"][0]["text"]
    assert "g1_safe_lie_to_stand" in text
    assert "preamble_s" in text
...
def test_a_zero_preamble_is_refused_because_it_collapses_to_a_bare_lie_to_stand() -> None:
    """A ``preamble_s=0.0`` is refused before the driver is called.

    A zero-length preamble collapses to a bare ``Lie2StandUp``
    write, which the neon bundle's own wrapper documented as a
    distinct ``use_unitree(service='loco',
    operation='Lie2StandUp', ...)`` verb.  The shared
    ...
    """
    driver = _StubG1Driver(envelope={"status": "success", "content": [{"json": {}}]})
    result = g1_safe_lie_to_stand(driver=driver, preamble_s=0.0)
    assert result["status"] == "error"
    text = result["content"][0]["text"]
    assert "g1_safe_lie_to_stand" in text
    assert "preamble_s" in text
...
def test_a_negative_preamble_is_refused_because_time_sleep_cannot_use_it() -> None:
    ...
    """
    driver = _StubG1Driver(envelope={"status": "success", "content": [{"json": {}}]})
    result = g1_safe_lie_to_stand(driver=driver, preamble_s=-0.5)
    assert result["status"] == "error"
    text = result["content"][0]["text"]
    assert "g1_safe_lie_to_stand" in text
    assert "preamble_s" in text
...
def test_a_boolean_preamble_is_refused_because_it_would_act_as_a_silent_float() -> None:
    ...
    ``Lie2StandUp`` write).  Refusing ``bool`` explicitly rather
    ...
    """
    driver = _StubG1Driver(envelope={"status": "success", "content": [{"json": {}}]})
    result = g1_safe_lie_to_stand(driver=driver, preamble_s=True)  # type: ignore[arg-type]
    assert result["status"] == "error"
    text = result["content"][0]["text"]
    assert "g1_safe_lie_to_stand" in text
    assert "preamble_s" in text
...
def test_a_non_finite_preamble_is_refused_before_the_driver_is_called() -> None:
    ...
    """
    driver = _StubG1Driver(envelope={"status": "success", "content": [{"json": {}}]})
    result = g1_safe_lie_to_stand(driver=driver, preamble_s=math.inf)
    assert result["status"] == "error"
    text = result["content"][0]["text"]
    assert "g1_safe_lie_to_stand" in text
    assert "preamble_s" in text
    ...
    result_nan = g1_safe_lie_to_stand(driver=driver_nan, preamble_s=math.nan)
    assert result_nan["status"] == "error"
    text_nan = result_nan["content"][0]["text"]
    assert "g1_safe_lie_to_stand" in text_nan
