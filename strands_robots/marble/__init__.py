...
class MarbleConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.model not in MARBLE_MODELS:
            raise ValueError(
                f"Invalid model '{self.model}'. " f"Must be one of: {MARBLE_MODELS}"
            )
        ...
        if self.api_key is None:
            self.api_key = os.environ.get("WLT_API_KEY") or os.environ.get(
                "MARBLE_API_KEY"
            )
...
class MarblePipeline:
    ...
    def _call_marble_api_remote(
        ...
        **kwargs: Any,
        ...
        if (
            ...
            and input_image
            and os.path.isfile(input_image)
        ):
            try:
        ...
        if (
            ...
            and input_video
            and os.path.isfile(input_video)
        ):
            try:
                media_asset_id = self._upload_media_asset(
    ...
    def _run_subprocess(self, cmd: List[str], label: str) -> str:
        ...
        if result.returncode != 0:
            ...
            raise RuntimeError(
                f"{label} failed (rc={result.returncode}): " f"{result.stderr[-500:]}"
            )
