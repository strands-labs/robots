...
This tool uses LeRobot's Teleoperator and Robot classes directly in-process.
    ...
    try:
        if action == "teleop":
            ...
            if not _ACTIVE_SESSION["running"] or not _ACTIVE_SESSION["record_session"]:
                return {
                    "status": "error",
                    "content": [{"text": "❌ No active recording session"}],
                }
            _ACTIVE_SESSION["record_session"].discard_episode()
            return {
                "status": "success",
                "content": [{"text": "🗑️ Current episode discarded"}],
            }
