...
def _discover_modules() -> Dict[str, Any]:
    ...
    key_apis = {
        ...
        "datasets.lerobot_dataset.LeRobotDataset.create": "Create new dataset (supports streaming_encoding, video_backend='auto')",
        "policies.factory.get_policy_class": "Get policy class by name (act, pi0, pi0_fast, smolvla, wall_x, xvla, diffusion, sarm...)",
        ...
        "configs.train.TrainPipelineConfig": "Training configuration (supports --policy.peft_config for LoRA)",
        ...
        "envs.factory.make_env": "Create gym environment (supports EnvHub: --env.type=hub --env.hub_path=...)",
    ...
    try:
        ...
        if module == "__discovery__":
            ...
            if "error" in discovery:
                return {
                    "status": "error",
                    "content": [{"text": f"❌ {discovery['error']}"}],
                }
        ...
        return {
            "status": "success",
            "content": [{"text": f"✅ {module}.{method}() → {serialized[:3000]}"}],
        }
