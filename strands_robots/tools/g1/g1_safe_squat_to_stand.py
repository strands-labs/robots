"""Agent-facing wrapper for ``G1Driver.safe_squat_to_stand``.

``G1Driver.safe_squat_to_stand`` is the driver-side compound-posture
entry point for the SQUAT->STAND transition: a caller passes a
...
:meth:`~unitree_sdk2py.g1.loco.g1_loco_client.LocoClient.Squat2StandUp`
...
a handle without a ``safe_squat_to_stand`` method refuses with a
...
through ``driver.safe_squat_to_stand`` and returns the envelope the
driver produced verbatim.  A future field the driver adds on the
success path (say, an ``rc`` from the SDK's ``Squat2StandUp``
...
an object without ``safe_squat_to_stand``) and the driver's own
...
``{3, 4, 706}`` or above the ``1.4`` rad pose threshold; the
...
import.  The verb is duck-typed on ``safe_squat_to_stand``; any
object with a synchronous ``safe_squat_to_stand(preamble_s)``
...
* Fire the FSM or pose guard the neon bundle's own
  ...
  refresher.
* Fall back to a bare ``LocoClient.Squat2StandUp`` when the Damp
  ...
  via ``use_unitree(service='loco', operation='Squat2StandUp',
...
def g1_safe_squat_to_stand(
    ...
    """Fire the Damp -> Squat2StandUp compound-posture transition.

    Calls ``G1Driver.safe_squat_to_stand`` once and returns the
    ...
    ``LocoClient.Squat2StandUp`` - the SDK's canonical SQUAT-to-
    ...
    Args:
        driver: The live :class:`~strands_robots.drivers.g1.G1Driver`
            ...
            without a callable ``safe_squat_to_stand`` accessor
            is refused by
        ...
        preamble_s: How long the driver's ``Damp`` preamble holds
            ...
            ``Squat2StandUp`` write, which the neon bundle's own
            ...
            silently collapses to a bare ``Squat2StandUp``) -
            neither is a value a caller writing ``True`` would
            have named on purpose.

    Returns:
        The envelope ``G1Driver.safe_squat_to_stand`` returned.
    ...
    # ``safe_squat_to_stand`` (a callable that fires a
    # ``LocoClient.Damp`` preamble and a follow-up
    # ``LocoClient.Squat2StandUp``, then returns the driver's
    ...
    refusal = live_handle_refusal(
        ...
        driver,
        accessor="safe_squat_to_stand",
        reads=(
            "the verb requests a G1 Damp -> Squat2StandUp compound "
            "transition through the driver's own SDK-facing write "
            "path and reads back the envelope the driver produced"
        ),
        expected=(
            "a callable ``safe_squat_to_stand(preamble_s)`` "
    ...
    # the driver's own ``safe_squat_to_stand`` would surface as an
    ...
    # collapses to a bare ``Squat2StandUp`` write, which the neon
    # bundle's own wrapper documented as a distinct
    # ``use_unitree`` verb.
    preamble_error = positive_finite_number_error(preamble_s, "preamble_s", "g1_safe_squat_to_stand")
    if preamble_error is not None:
        return _refusal_envelope(preamble_error)

    return driver.safe_squat_to_stand(preamble_s)
